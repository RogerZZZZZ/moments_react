function ajaxLoc(conf) {
	var jsonpCallback = 'egg' + (+new Date());
	...
	var src = conf.url.split('?')[0] + '?callback=' + jsonpCallback + '&' + conf.url.split('?')[1];
	...
	timerHandler = window.setTimeout(timeoutHandler, 10 * 1000);

}
function ajaxLoc1(conf) {
	var jsonpCallback = 'egg' + (+new Date());
	// var params = conf.params || {};
	// params['jsonp'] = jsonpCallback;
	var timerHandler = null;
	...
	script.type = 'text/javascript';
	script.id = jsonpCallback;
	var src = conf.url.split('?')[0] + '?callback=' + jsonpCallback;
	//var src = conf.url.split('?')[0] + '?callback=' + jsonpCallback + '&' + conf.url.split('?')[1];
	script.src = src.substr(0, 1900);
	script.charset = 'utf-8';
	script.onerror = function() {
